...
class GenerateBrandGradientDrawables(
    ...
    private fun generateGradientDrawableFile(tokenName: String, brandName: String, isDark: Boolean) {
        ...
        val drawableDir = if (isDark) {
            GenerateXMLFiles.DRAWABLE_NIGHT_DIR
        } else {
            GenerateXMLFiles.DRAWABLE_DIR
        }
        File(
            "$drawableDir/${getGradientDrawableResourceName(brandName, tokenName)}.xml"
        ).writeText(gradientResourcesXml.toString(PrintOptions(singleLineTextElements = true)))
    }

    private fun getGradientClassName(tokenName: String, brandName: String, isDark: Boolean): String =
    ...
    private companion object {
        const val BRAND_GRADIENTS_FILENAME_SUFIX = "BrandGradientsAutogenerated"

        val LINEAR_GRADIENT_WITH_ANGLE_DRAWABLE_CLASS = ClassName(GRADIENT_PACKAGE, "LinearGradientWithAngleDrawable")
        val R_CLASS = ClassName("com.telefonica.mistica", "R")
    }
}
