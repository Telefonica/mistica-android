...
class GenerateXMLFiles(
    ...
    private fun Node.mapColors(colors: Map<String, BrushDTO.SolidColorDTO>, brand: Brand) {
        colors.forEach { color ->
            ...
            "item" {
                attribute("name", colorName)
                -"@color/${getColorValue(color.value.value, brand.name)}"
            }
        }
    }
        ...
        brushes.forEach { brushEntry ->
            ...
            when (brush) {
                is BrushDTO.SolidColorDTO -> {
                    colorStops.add(0F)
                    colorValues.add(getColorValue(brush.value, brand.name))
                }
                is BrushDTO.GradientDTO -> {
                    angle = brush.value.angle
                    brush.value.colors.forEach { color ->
                        colorStops.add(color.stop)
                        colorValues.add(getColorValue(color.value, brand.name))
                    }
                }
            }
    ...
    private fun getColorValue(
        ...
        val paletteColorName = TokensGenerator.COLOR_NAME_REGEX.find(color)?.groups?.get(1)?.value
        var colorValue = "${brandName}_color_${paletteColorName}"
        if (color.contains("rgba(")) {
            val alpha = ALPHA_REGEX.find(color)?.value?.toDouble()
            if (alpha != null && paletteColorName != null) {
                colorValue = getColorNameWithAlpha(brandName, paletteColorName, alpha)
            }
        }
        return colorValue
    }
